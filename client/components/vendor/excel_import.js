           ...
           let productsArray = []
            ...
            if (range[0].length > 0 && range[1].length > 0){  
                for (let i=0; i<range[0].length; i++){                   
                    if (range[0][i]){
                        if (numberArray.includes(range[0][i]))
                        {row[0] += range[0][i].toString();}else{collumn[0] += range[0][i].toUpperCase();}
                    }
                }
                for (let i=0; i<range[1].length; i++){                                    
                    if (range[1][i]){
                        if (numberArray.includes(range[1][i]))
                        {row[1] += range[1][i].toString();}else{collumn[1] += range[1][i].toUpperCase();}
                    }
                }
            }else{ return {error:'Verifique se a planillha foi preenchida corretamente, ou se selecionou o arquivo correto.'}}
            let sheetLimits = {rows:row, collumns:collumn};
            console.log(sheetLimits);
            row = [parseInt(row[0])-1, parseInt(row[1])-1];
            collumn = [letterArray.indexOf(collumn[0]), letterArray.indexOf(collumn[1])];            
            let indexLimits = {rows:row, collumns:collumn};
