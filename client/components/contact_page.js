...
class ContactPage extends Component {
    ...
    validateInputs(){
        ...
        if (this.state.mensagem.value.length < 10){
            this.errors.push('Sua mensagem deve ter no mínimo 10 caracteres.');
            this.setState({loading: false});
            return;
        }
        Meteor.call('sendMessage', this.state.mensagem.value, (error)=>{
            let mensagem = this.state.mensagem;
            if (error){
                console.log(error)
                this.errors.push(error.reason);
            }else{
                this.successText = 'Sua mensagem foi enviada com sucesso, aguarde que em breve iremos entrar em contato.';                
                mensagem.value = '';
                mensagem.border = 'white';
                mensagem.input = 'whiteInput';
                mensagem.background = 'transparent';                
            }
            this.setState({loading: false, mensagem: mensagem});
        })
    }
    displayErrors(){      
        ...
        return(<div style={{margin:'10px 0'}}>
            {this.errors.map((error, index)=>{
                ...
                return(<div style={{color:'white', fontSize:'14px'}} key={key}>{error}</div>)
            })}
        </div>)}        
    }    
    render(){
        //f (!Meteor.userId()){ Meteor.logout(); history.push('/entrar'); }
        return(
        <div>
            <VendorHeader/>
                    ...
                    <div style={{margin:'10px 20px', padding:'5px 10px', borderRadius:'3px', backgroundColor:'#F7F7F7'}}>
                        ...
                        <div style={{width:'100%', maxWidth:'570px', margin:'0 auto', marginTop:'30px'}}>
                            <div style={{margin:'35px 20px', marginBottom:'0px', padding:'15px 20px', border:'1px solid #ff7000', borderBottom:'0px', backgroundColor:'white'}}>
                                ...
                                <div style={{width:'fit-content', margin:'10px auto', fontSize:'14px', color:'#444', lineHeight:'22px'}}>
                                    <div style={{width:'fit-content', display:'flex'}}><div style={{width:'16px', height:'16px', margin:'auto 0', marginRight:'5px', backgroundImage:'url(/imgs/icons/icon-whatsapp.png)', backgroundPosition:'center', backgroundRepeat:'no-repeat', backgroundSize:'contain'}}></div>(11) 99510-7345</div>
                                    <div style={{width:'fit-content', display:'flex'}}><div style={{width:'16px', height:'16px', margin:'auto 0', marginRight:'5px', backgroundImage:'url(/imgs/icons/icon-whatsapp.png)', backgroundPosition:'center', backgroundRepeat:'no-repeat', backgroundSize:'contain'}}></div>(11) 93407-8531</div>
                                    <div style={{width:'fit-content', display:'flex'}}><div style={{width:'16px', height:'16px', margin:'auto 0', marginRight:'5px', backgroundImage:'url(/imgs/icons/icon-telefone.png)', backgroundPosition:'center', backgroundRepeat:'no-repeat', backgroundSize:'contain'}}></div>(11) 4226-7099</div>
                                    <div style={{width:'fit-content', display:'flex'}}><div style={{width:'16px', height:'16px', margin:'auto 0', marginRight:'5px', backgroundImage:'url(/imgs/icons/icon-telefone.png)', backgroundPosition:'center', backgroundRepeat:'no-repeat', backgroundSize:'contain'}}></div>(11) 4318-7223</div>    
                                </div> 
                                ...
                                <div style={{marginTop:'8px', marginBottom:'10px', fontSize:'15px', color:'#444'}}>
                            ...
                            <div style={{margin:'20px', marginTop:'0px',  backgroundColor:'#ff7000'}}>
                                <div style={{height:'30px', backgroundImage:'url(/imgs/others/whiteDownArrow.png)', backgroundPosition:'center', backgroundRepeat:'no-repeat', backgroundSize:'contain'}}></div>
                                <div style={{margin:'20px'}}>
                                    ...
                                    <div style={{marginTop:'35px', padding:'0 20px'}}>                                                                
                                        ...
                                        <div style={{fontSize:'13px', paddingTop:'15px', color:'white', display: (this.successText == '') ? 'none' : 'block' }}>{this.successText}</div>
                                        <div style={{fontSize:'13px', paddingTop:'15px', color:'white', display: (this.errors.length == 0) ? 'none' : 'block' }}>{this.errors[0]}</div>
